...
void Delete_p_q(PSeqList s, int p, int q) {
	//删除有序表中值为p,q(p<q)之间的元素，因为是有序，所以p,q之间的元素连续
	int i, j;
...
int M_Search(int a[], int b[], int n) {//两个升序数组，合并后返回其中位数
	...
	while (s1 != d1 || s2 != d2) {
		...
		if (a[m1] == b[m2])
			return a[m1];
		else if (a[m1] < b[m2]) {//舍弃a中小的一半，舍弃b中大的一半
